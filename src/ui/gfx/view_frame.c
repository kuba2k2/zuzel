...
static void gfx_view_inflate_frame(view_t *frame, cJSON *json) {
	if (frame->data.frame.children != NULL)
		LT_ERR(E, return, "Frame children already inflated");
	frame->data.frame.children = gfx_view_inflate(cJSON_GetObjectItem(json, "children"));
}
...
static void gfx_view_measure_frame(view_t *frame) {
	...
	LL_FOREACH(frame->data.frame.children, child) {
		if (child->is_gone)
			continue;
...
static void gfx_view_draw_frame(SDL_Renderer *renderer, view_t *frame) {
	gfx_view_draw(renderer, frame->data.frame.children);
}
